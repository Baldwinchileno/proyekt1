...
namespace AdminSERMAC.Forms
{
    public class InventarioForm : Form
    {
        ...
        private void ModificarProductoButton_Click(object sender, EventArgs e)
        {
            try
            {
                // Primero mostrar un diálogo para ingresar el código del producto
                using (var inputBox = new TextBox())
                {
                    var result = MessageBox.Show(inputBox, "Ingrese el código del producto",
                        "Modificar Producto", MessageBoxButtons.OKCancel);

                    if (result == DialogResult.OK && !string.IsNullOrEmpty(inputBox.Text))
                    {
                        var modificarForm = new ModificarProductoForm(
                            inputBox.Text,
                            (ILogger<SQLiteService>)_logger, // Usar el logger existente del formulario
